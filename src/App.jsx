...
export default function App() {
  ...
  const [umpireSearch, setUmpireSearch] = useState(''); // Separate search for name picker
  ...
  const myInterestedGames = useMemo(() => {
    if (!umpireId) return [];
    const interestedIds = applications.filter(a => a.userId === umpireId).map(a => a.gameId);
    return games.filter(game => interestedIds.includes(game.id) && !myAssignedGames.some(ag => ag.id === game.id));
  }, [games, applications, umpireId, myAssignedGames]);
  ...
  const filteredMasterUmpires = useMemo(() => {
    if (!umpireSearch && !isAddingNew) return masterUmpires;
    return masterUmpires.filter(u => u.name.toLowerCase().includes(umpireSearch.toLowerCase()));
  }, [masterUmpires, umpireSearch, isAddingNew]);
  ...
  const saveEditedGame = async () => {
    ...
    } catch (e) { console.error("Update failed", e); alert("Error updating match."); }
  };
  ...
  const CalendarGrid = ({ gamesToShow, isPersonal = false }) => {
    return (
      <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm animate-in fade-in duration-300">
        ...
        <div className="grid grid-cols-[50px_repeat(7,1fr)] border-b border-slate-100">
          <div className="py-3 text-center text-[10px] font-black text-slate-300 uppercase tracking-widest bg-slate-50/20">{t.weekAbbr}</div>
          {[1, 2, 3, 4, 5, 6, 0].map(d => (
            <div key={d} className="py-3 text-center text-[10px] font-black text-slate-400 uppercase tracking-widest border-l border-slate-50">{t.days[d]}</div>
          ))}
        </div>
        <div className="grid grid-cols-[50px_repeat(7,1fr)]">
          {calendarDays.map((day, idx) => {
            const dateStr = day ? day.toISOString().split('T')[0] : null;
            const matches = dateStr ? gamesToShow.filter(g => g.date === dateStr) : [];
            ...
            return (
              <React.Fragment key={idx}>
                {isWeekStart && (
                  <div className="border-b border-slate-50 flex items-center justify-center bg-slate-50/10 text-[10px] font-black text-slate-300">
                    {day ? getISOWeekNumber(day) : ''}
                  </div>
                )}
                <div className={`min-h-[100px] p-2 border-l border-b border-slate-50 relative ${!day ? 'bg-slate-50/30' : 'bg-white'}`}>
                  {day && (
                    <>
                      ...
                      <div className="mt-2 space-y-1">
                        {matches.map(g => {
                          const isAssigned = isPersonal && myAssignedGames.some(ag => ag.id === g.id);
                          return (
                            <button 
                              key={g.id} 
                              onClick={() => {
                                  setSearchQuery(g.home);
                                  isPersonal ? setMyGamesViewMode('list') : setScheduleViewMode('list');
                              }}
                              className={`w-full text-left p-1 rounded border transition-all overflow-hidden ${isAssigned ? 'bg-green-50 border-green-100' : 'border-slate-100 bg-white'}`}
                            >
                              <div className={`w-full h-1 rounded-full mb-1 ${isAssigned ? 'bg-green-500' : getLeagueStyles(g.league).split(' ')[0]}`} />
                              <p className={`text-[8px] font-bold truncate leading-none uppercase ${isAssigned ? 'text-green-700' : 'text-slate-700'}`}>{g.away} @ {g.home}</p>
                            </button>
                          );
                        })}
                      </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-24 selection:bg-blue-100">
        ...
        <div className="max-w-5xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="bg-white rounded-lg overflow-hidden flex items-center justify-center h-10 w-10 border border-white/20 shadow-inner">
              {LOGO_URL ? <img src={LOGO_URL} alt="Logo" className="h-full w-full object-contain p-1" /> : <Trophy className="w-6 h-6 text-blue-900" />}
            </div>
            <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <select value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} className="bg-blue-800 text-[10px] font-black uppercase border-none rounded-lg px-2 py-1 outline-none appearance-none cursor-pointer"><option value="2025">2025</option><option value="2026">2026</option><option value="2027">2027</option></select>
      ...
      <main className="max-w-5xl mx-auto p-4 space-y-6">
        ...
        <div className="flex bg-white p-1.5 rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          {[
            ...
            <button key={tab.id} onClick={() => setView(tab.id)} className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[10px] font-black uppercase transition-all duration-200 ${view === tab.id ? 'bg-blue-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>
        ...
        {(view === 'schedule' || view === 'admin') && (
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><input type="text" placeholder={t.searchPlaceholder} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all" /></div>
        ...
        <section className="space-y-4">
          {view === 'schedule' && (
            <>
              ...
              {scheduleViewMode === 'calendar' ? <CalendarGrid gamesToShow={filteredGames} /> : (
                filteredGames.length === 0 ? <div className="bg-white p-16 rounded-3xl text-center border-2 border-dashed border-slate-200"><Info className="w-12 h-12 text-slate-200 mx-auto mb-4" /><p className="text-slate-500 font-medium">{t.noGames}</p></div> : (
                      ...
                      <div className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="flex gap-4">
                          <div className="bg-slate-50 p-3 rounded-xl text-center min-w-[75px] border border-slate-100">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.days[new Date(game.date).getDay()]}</p>
                            <p className="text-2xl font-black text-slate-800 leading-none">{new Date(game.date).getDate()}</p>
                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-tighter mt-0.5">{new Date(game.date).toLocaleDateString(lang === 'sv' ? 'sv-SE' : 'en-US', { month: 'short' })}</p>
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                            ...
                            {groupedAssignments[game.id]?.length > 0 && <div className="flex flex-wrap gap-1 mt-3 items-center">{groupedAssignments[game.id].map(asg => (<div key={asg.userId} className="bg-green-50 text-green-700 text-[10px] font-bold px-2 py-1 rounded-lg border border-green-100 flex items-center gap-1"><CheckCircle className="w-3 h-3" /> {asg.userName}</div>))}</div>}
                          </div>
                        </div>
                        <div className="flex items-center justify-between sm:flex-col sm:items-end gap-3 pt-3 sm:pt-0 border-t sm:border-t-0 border-slate-50">
                          {!showHistory && (
                            <><div className="flex flex-col items-end"><span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">{applications.filter(a => a.gameId === game.id).length} {t.applied}</span>{groupedAssignments[game.id]?.length > 0 && <span className="text-[10px] font-black text-green-600 uppercase tracking-widest mt-0.5">{groupedAssignments[game.id].length}/4 {t.staffed}</span>}</div><button onClick={() => toggleApplication(game.id)} disabled={umpireId && groupedAssignments[game.id]?.some(asg => asg.userId === umpireId)} className={`px-6 py-2 rounded-xl text-xs font-black uppercase transition-all ${umpireId && applications.some(a => a.gameId === game.id && a.userId === umpireId) ? 'bg-red-50 text-red-600 border border-red-100' : 'bg-blue-600 text-white shadow-lg active:scale-95'}`}>{umpireId && applications.some(a => a.gameId === game.id && a.userId === umpireId) ? t.withdraw : t.interested}</button></>
                          )}
                        </div>
                      </div>
                    </div>
          ...
          {view === 'admin' && (
            <div className="space-y-6">
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-blue-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div className="flex gap-2"><button onClick={deleteAllGames} className="bg-red-50 text-red-600 border border-red-100 px-6 py-3 rounded-2xl font-bold text-xs uppercase hover:bg-red-100 transition-all active:scale-95">{t.deleteAllGames}</button><button onClick={() => setShowImportTool(!showImportTool)} className="bg-blue-900 text-white px-6 py-3 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-black transition-all shadow-lg"><Plus className="w-4 h-4" /> {t.bulkImport}</button></div>
              </div>
              {showImportTool && (<div className="bg-blue-50 p-6 rounded-3xl border border-blue-200 animate-in slide-in-from-top"><h3 className="font-bold text-blue-800 mb-2 flex items-center gap-2"><FileText className="w-4 h-4" /> {t.pasteSheet}</h3><textarea value={bulkInput} onChange={(e) => setBulkInput(e.target.value)} placeholder="YYYY-MM-DD	HH:MM	Serie	Borta	Hemma	Plats" className="w-full h-40 p-4 bg-white border border-blue-200 rounded-xl font-mono text-xs mb-4 outline-none" /><div className="flex gap-3"><button onClick={handleBulkImport} className="flex-1 bg-blue-700 text-white py-3 rounded-xl font-black uppercase text-xs">{t.addGames}</button><button onClick={() => setShowImportTool(false)} className="px-6 py-3 bg-white border border-blue-200 text-blue-600 rounded-xl font-black uppercase text-xs">{t.cancel}</button></div></div>)}
              <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm"><h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2"><Users className="w-4 h-4" /> {t.masterList}</h3><div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">{masterUmpires.map(u => (<div key={u.id} className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 border border-slate-100">{editingUmpireId === u.id ? (<div className="flex flex-1 gap-2"><input type="text" value={tempEditName} onChange={(e) => setTempEditName(e.target.value)} className="flex-1 bg-white border border-blue-300 px-3 py-1 rounded-lg text-sm font-bold outline-none" /><button onClick={async () => { await updateMasterUmpire(u.id, tempEditName); setEditingUmpireId(null); }} className="bg-green-600 text-white p-1.5 rounded-lg"><Check className="w-4 h-4" /></button><button onClick={() => setEditingUmpireId(null)} className="bg-slate-200 text-slate-600 p-1.5 rounded-lg"><UserMinus className="w-4 h-4" /></button></div>) : (<><span className="text-sm font-bold text-slate-700">{u.name}</span><div className="flex gap-1"><button onClick={() => { setEditingUmpireId(u.id); setTempEditName(u.name); }} className="p-1.5 text-slate-400 hover:text-blue-600"><Edit2 className="w-4 h-4" /></button><button onClick={() => deleteMasterUmpire(u.id)} className="p-1.5 text-slate-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button></div></>)}</div>))}</div></div>
              <div className="space-y-4"><div className="flex justify-between items-center px-2"><h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">{t.pendingAssignments}</h3><button onClick={() => setShowStaffed(!showStaffed)} className="text-[10px] font-black text-blue-600 uppercase hover:underline">{showStaffed ? t.hideStaffed : t.showAll}</button></div>
                {filteredGames.filter(g => showStaffed ? true : (groupedAssignments[g.id]?.length || 0) < 4).map(game => (
                  <div key={game.id} className={`bg-white rounded-2xl border overflow-hidden shadow-sm ${groupedAssignments[game.id]?.length >= 4 && editingGameData?.id !== game.id ? 'opacity-60 grayscale' : 'border-slate-200'}`}>
                    <div className="p-4 bg-slate-50/50 border-b border-slate-100 flex justify-between items-center"><div className="flex items-center gap-3 flex-wrap"><span className={`text-[10px] font-black px-2 py-0.5 rounded border uppercase tracking-widest ${getLeagueStyles(game.league)}`}>{game.league}</span><p className="text-xs font-bold text-slate-600">{game.away} @ {game.home} | {game.date}</p><span className={`text-[10px] font-black px-2 py-0.5 rounded border uppercase ${getAssignmentStatusStyles(groupedAssignments[game.id]?.length || 0)}`}>{groupedAssignments[game.id]?.length || 0} / 4 {t.assignedTo}</span></div><div className="flex items-center gap-1"><button onClick={() => setEditingGameData(editingGameData?.id === game.id ? null : { ...game })} className={`p-2 transition-colors ${editingGameData?.id === game.id ? 'text-blue-600' : 'text-slate-400 hover:text-blue-500'}`}><Edit2 className="w-4 h-4" /></button><button onClick={() => { if(window.confirm(t.deleteConfirm)) deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'games', game.id)); }} className="p-2 text-slate-300 hover:text-red-500"><Trash2 className="w-4 h-4" /></button></div></div>
                    {editingGameData?.id === game.id ? (
                      <div className="p-6 bg-blue-50/30 space-y-4 animate-in fade-in zoom-in duration-200"><div className="grid grid-cols-2 gap-3"><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 pl-1">{t.date}</label><input type="date" value={editingGameData.date} onChange={(e) => setEditingGameData({ ...editingGameData, date: e.target.value })} className="w-full p-2 bg-white border border-slate-200 rounded-lg text-sm font-bold" /></div><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 pl-1">{t.time}</label><input type="text" value={editingGameData.time} onChange={(e) => setEditingGameData({ ...editingGameData, time: e.target.value })} className="w-full p-2 bg-white border border-slate-200 rounded-lg text-sm font-bold" /></div><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 pl-1">{t.away}</label><input type="text" value={editingGameData.away} onChange={(e) => setEditingGameData({ ...editingGameData, away: e.target.value })} className="w-full p-2 bg-white border border-slate-200 rounded-lg text-sm font-bold" /></div><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 pl-1">{t.home}</label><input type="text" value={editingGameData.home} onChange={(e) => setEditingGameData({ ...editingGameData, home: e.target.value })} className="w-full p-2 bg-white border border-slate-200 rounded-lg text-sm font-bold" /></div><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 pl-1">{t.league}</label><input type="text" value={editingGameData.league} onChange={(e) => setEditingGameData({ ...editingGameData, league: e.target.value })} className="w-full p-2 bg-white border border-slate-200 rounded-lg text-sm font-bold" /></div><div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 pl-1">{t.location}</label><input type="text" value={editingGameData.location} onChange={(e) => setEditingGameData({ ...editingGameData, location: e.target.value })} className="w-full p-2 bg-white border border-slate-200 rounded-lg text-sm font-bold" /></div></div><div className="flex gap-2"><button onClick={saveEditedGame} className="flex-1 bg-green-600 text-white py-3 rounded-xl font-black uppercase text-[10px] tracking-widest shadow-lg shadow-green-100">{t.saveChanges}</button><button onClick={() => setEditingGameData(null)} className="px-6 bg-slate-200 text-slate-600 py-3 rounded-xl font-black uppercase text-[10px] tracking-widest">{t.cancel}</button></div></div>
                    ) : (
                      <div className="p-4 space-y-4">{groupedAssignments[game.id]?.length > 0 && (<div className="space-y-1"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t.crew}</p><div className="grid grid-cols-1 sm:grid-cols-2 gap-2">{groupedAssignments[game.id].map(asg => (<div key={asg.userId} className="flex items-center justify-between p-2 rounded-xl border border-green-100 bg-green-50/30"><div className="flex items-center gap-2"><Users className="w-3 h-3 text-green-600" /><span className="text-xs font-bold text-slate-700">{asg.userName}</span></div><button onClick={() => removeAssignment(game.id, asg.userId)} className="text-red-500 hover:bg-red-50 p-1.5 rounded-lg transition-colors"><UserMinus className="w-3.5 h-3.5" /></button></div>))}</div></div>)}<div className="space-y-1"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t.interests}</p>{applications.filter(a => a.gameId === game.id && !groupedAssignments[game.id]?.some(asg => asg.userId === a.userId)).length === 0 ? <p className="text-xs text-slate-400 italic">{t.noInterest}</p> : (<div className="grid grid-cols-1 sm:grid-cols-2 gap-2">{applications.filter(a => a.gameId === game.id && !groupedAssignments[game.id]?.some(asg => asg.userId === a.userId)).map(app => (<div key={app.userId} className="flex items-center justify-between p-2 rounded-xl border border-slate-100 bg-white hover:border-blue-300 transition-all"><span className="text-xs font-bold">{app.userName}</span><button disabled={groupedAssignments[game.id]?.length >= 4} onClick={() => assignUmpire(game.id, app.userId, app.userName)} className="bg-blue-600 text-white text-[10px] font-black uppercase px-3 py-1.5 rounded-lg hover:bg-blue-700 flex items-center gap-1.5 disabled:opacity-50"><UserPlus className="w-3 h-3" /> Assign</button></div>))}</div>)}</div></div>
                    )}
                  </div>
                ))}
              </div>
          ...
          {view === 'stats' && (
            <div className="space-y-6">
              <div className="bg-blue-900 text-white p-8 rounded-3xl shadow-xl flex items-center justify-between"><div><h2 className="text-2xl font-black tracking-tighter">{t.analytics}</h2><p className="text-blue-200 text-xs font-bold uppercase tracking-widest">{selectedYear} Season Engagement</p></div><BarChart3 className="w-12 h-12 opacity-20" /></div>
              <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm"><table className="w-full text-left"><thead><tr className="bg-slate-50 border-b border-slate-100"><th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.umpire}</th><th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">{t.interests}</th><th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">{t.gamesAssigned}</th></tr></thead><tbody className="divide-y divide-slate-50">{statistics.map(stat => (
                      <tr key={stat.name} className="hover:bg-slate-50 transition-colors"><td className="px-6 py-4"><div className="flex items-center gap-3"><div className="w-8 h-8 bg-blue-100 text-blue-700 rounded-full flex items-center justify-center text-xs font-black uppercase">{stat.name.charAt(0)}</div><span className="font-bold text-slate-700">{stat.name}</span></div></td><td className="px-6 py-4 text-center"><span className="bg-slate-100 px-3 py-1 rounded-full text-xs font-black text-slate-500">{stat.interest}</span></td><td className="px-6 py-4 text-center"><span className="bg-blue-100 px-3 py-1 rounded-full text-xs font-black text-blue-700">{stat.games}</span></td></tr>))}</tbody></table>{statistics.length === 0 && <p className="p-12 text-center text-slate-400 italic text-sm">{t.noStats}</p>}</div>
            </div>
          )}
          ...
          {view === 'my-apps' && (
            <div className="space-y-4">
              ...
              {myGamesViewMode === 'calendar' ? (
                <CalendarGrid gamesToShow={[...myAssignedGames, ...myInterestedGames]} isPersonal />
              ) : (
                <>
                    ...
                    <div className="space-y-3">
                      ...
                      {myAssignedGames.map(game => (
                        <div key={game.id} className="bg-white p-4 rounded-2xl shadow-sm border border-green-200 bg-green-50/30 transition-all flex items-center justify-between">
                          <div className="flex items-center gap-4"><div className="p-3 rounded-xl bg-green-100"><CalendarIcon className="w-5 h-5 text-green-600" /></div><div><div className="flex items-center gap-2"><span className={`text-[8px] font-black px-1.5 py-0.5 rounded border uppercase ${getLeagueStyles(game.league)}`}>{game.league}</span><p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">{t.days[new Date(game.date).getDay()]} {new Date(game.date).toLocaleDateString(lang === 'sv' ? 'sv-SE' : 'en-US', { day: 'numeric', month: 'short' })} @ {game.time}</p></div><p className="font-bold text-slate-900 leading-tight text-base mt-1">{game.away} @ {game.home}</p></div></div>
                  ...
                  <div className="space-y-3">
                    ...
                    {myInterestedGames.length === 0 ? (myAssignedGames.length === 0 && <div className="bg-white p-16 rounded-3xl text-center border-2 border-dashed border-slate-200"><div className="bg-slate-50 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4"><CalendarIcon className="w-8 h-8 text-slate-300" /></div><p className="text-slate-500 font-medium">{t.noInterest}</p></div>) : (
                      myInterestedGames.map(game => (
                        <div key={game.id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex items-center justify-between">
                            <div className="flex items-center gap-4"><div className="p-3 rounded-xl bg-slate-100"><CalendarIcon className="w-5 h-5 text-slate-400" /></div><div><div className="flex items-center gap-2"><span className={`text-[8px] font-black px-1.5 py-0.5 rounded border uppercase ${getLeagueStyles(game.league)}`}>{game.league}</span><p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">{t.days[new Date(game.date).getDay()]} {new Date(game.date).toLocaleDateString(lang === 'sv' ? 'sv-SE' : 'en-US', { day: 'numeric', month: 'short' })} @ {game.time}</p></div><p className="font-bold text-slate-900 leading-tight text-base mt-1">{game.away} @ {game.home}</p></div></div>
                            <div className="flex items-center gap-2"><button onClick={() => handleCalendarExport(game)} className="p-2 text-blue-600 hover:bg-blue-100 rounded-lg transition-colors" title={t.addToCalendar}><CalendarPlus className="w-5 h-5" /></button><button onClick={() => toggleApplication(game.id)} className="text-slate-300 hover:text-red-500 p-2.5 transition-colors"><Trash2 className="w-5 h-5" /></button></div>
                        </div>
                      ))
                    )}
                  </div>
      ...
      {showNamePrompt && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-[2.5rem] p-8 space-y-6 max-w-sm w-full shadow-2xl animate-in fade-in zoom-in border border-white/20">
            ...
            <div className="space-y-4">
              <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">{t.masterList}</label><div className="relative group"><Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300 group-focus-within:text-blue-500 transition-colors" /><input type="text" value={umpireSearch} placeholder={t.namePlaceholder} onChange={(e) => setUmpireSearch(e.target.value)} className="w-full p-4 pl-11 bg-slate-50 border border-slate-200 rounded-2xl font-bold outline-none focus:ring-4 focus:ring-blue-500/10 transition-all text-sm" /></div><div className="mt-2 bg-slate-50 border border-slate-200 rounded-2xl max-h-48 overflow-y-auto divide-y divide-slate-100 custom-scrollbar">{filteredMasterUmpires.length > 0 ? filteredMasterUmpires.map(u => (<button key={u.id} onClick={async () => { setUserName(u.name); setUmpireId(u.id); await updateProfile(u.name, u.id); setShowNamePrompt(false); setUmpireSearch(''); }} className="w-full text-left px-4 py-3 hover:bg-blue-50 transition-colors flex items-center justify-between group"><span className="text-sm font-bold text-slate-700">{u.name}</span><ChevronRight className="w-4 h-4 text-slate-300 opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all" /></button>)) : <div className="p-4 text-center"><p className="text-xs text-slate-400 font-medium italic">{t.noGames}</p></div>}</div></div>
              <div className="pt-4 border-t border-slate-100 space-y-3"><button onClick={() => setIsAddingNew(!isAddingNew)} className="flex items-center gap-2 text-xs font-black text-blue-600 uppercase hover:underline"><Plus className="w-3 h-3" /> {t.addNewName}</button>{isAddingNew && (<div className="space-y-2 animate-in slide-in-from-top-2 duration-200"><input type="text" autoFocus value={tempEditName} onChange={(e) => setTempEditName(e.target.value)} placeholder="För- och efternamn" className="w-full p-3 bg-white border border-blue-200 rounded-xl font-bold text-sm outline-none" /><button onClick={async () => { if (tempEditName.trim()) { const newId = await addMasterUmpire(tempEditName); setUserName(tempEditName); setUmpireId(newId); await updateProfile(tempEditName, newId); setTempEditName(''); setIsAddingNew(false); setShowNamePrompt(false); } }} className="w-full py-3 bg-blue-600 text-white font-black rounded-xl text-[10px] uppercase tracking-widest shadow-lg shadow-blue-200">{t.createUmpire}</button></div>)}</div>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-[2.5rem] p-8 space-y-8 max-w-sm w-full shadow-2xl animate-in zoom-in border border-white/20 overflow-y-auto max-h-[90vh]">
            ...
            <div className="space-y-4">
              ...
              <div className="pt-6 border-t border-slate-100"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">{t.adminVerify}</label>{!isAdmin ? (<div className="flex gap-2 mt-2"><input type="password" placeholder={t.accessCode} value={adminCode} onChange={(e) => setAdminCode(e.target.value)} className="flex-1 p-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold outline-none" /><button onClick={handleAdminAuth} className="bg-slate-800 text-white px-5 rounded-2xl font-black uppercase text-xs hover:bg-black transition-all">{t.verify}</button></div>) : (<div className="p-4 bg-blue-50 rounded-2xl border border-blue-100 text-center mt-2"><p className="text-blue-700 font-black text-xs uppercase flex items-center justify-center gap-2"><Shield className="w-4 h-4" /> {t.adminActive}</p><button onClick={async () => { setIsAdmin(false); await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'profile', 'info'), { isAdmin: false }, { merge: true }); }} className="text-[10px] font-black text-red-500 uppercase hover:underline mt-2.5 block w-full text-center">{t.logoutAdmin}</button></div>)}</div>
            </div>
      ...
      <button onClick={() => setShowNamePrompt(true)} className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-blue-900 text-white px-6 py-3.5 rounded-full shadow-2xl flex items-center gap-5 z-40 border border-blue-800/50 backdrop-blur-md hover:scale-105 active:scale-95 transition-all">
        <div className="flex items-center gap-3"><div className="w-8 h-8 bg-white text-blue-900 rounded-full flex items-center justify-center text-[11px] font-black uppercase shadow-inner overflow-hidden">{userName ? userName.charAt(0) : '?'}</div><div className="text-left"><p className="text-[8px] font-black uppercase text-blue-300 leading-none mb-0.5">{userName ? t.status : t.setProfile}</p><span className="text-sm font-bold whitespace-nowrap leading-none">{userName || t.selectFromList}</span></div></div><div className="h-4 w-px bg-blue-700" /><div className="flex flex-col items-center"><span className="text-[10px] font-black uppercase text-blue-300 leading-none">{t.applied}</span><span className="text-[11px] font-bold leading-none mt-0.5">{umpireId && applications.filter(a => a.userId === umpireId).length}</span></div>
      </button>
    </div>
  );
}
